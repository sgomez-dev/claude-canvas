...
import { encodeKitty, encodeKittyClear, imageIdFor, MAX_CHUNK_BASE64 } from "./kitty";
...
test("a small payload is one escape carrying all the control data", () => {
  const parts = escapes(join(encodeKitty(small, { columns: 10, rows: 4, imageId: 42 })));
  expect(parts).toHaveLength(1);
  expect(parts[0]!.control).toBe("a=T,f=100,q=2,C=1,c=10,r=4,i=42,m=0");
...
test("responses are suppressed and the cursor is left alone", () => {
  const control = escapes(join(encodeKitty(small, { columns: 1, rows: 1, imageId: 42 })))[0]!.control;
  expect(control).toContain("q=2");
  expect(control).toContain("C=1");
});

test("the cell box is what the caller asked for", () => {
  const control = escapes(join(encodeKitty(small, { columns: 76, rows: 22, imageId: 42 })))[0]!.control;
...
test("a payload over the chunk limit is split, and no chunk exceeds it", () => {
  ...
  const parts = escapes(join(encodeKitty(big, { columns: 8, rows: 8, imageId: 42 })));
...
test("only the first chunk carries control data, and m marks the last", () => {
  const big = new Uint8Array(20_000);
  const parts = escapes(join(encodeKitty(big, { columns: 8, rows: 8, imageId: 42 })));
  expect(parts[0]!.control).toBe("a=T,f=100,q=2,C=1,c=8,r=8,i=42,m=1");
...
test("a payload landing exactly on the chunk boundary produces no empty tail", () => {
  ...
  const parts = escapes(join(encodeKitty(exact, { columns: 2, rows: 2, imageId: 42 })));
  expect(parts).toHaveLength(1);
...
test("an empty payload still produces one escape, not silence", () => {
  const parts = escapes(join(encodeKitty(new Uint8Array(0), { columns: 1, rows: 1, imageId: 42 })));
  expect(parts).toHaveLength(1);
  expect(parts[0]!.payload).toBe("");
});

// The placement is scoped to this canvas's own image id, so its later
// delete can target only its own placement -- not every image on the
// terminal. See encodeKittyClear and imageIdFor below.
test("the placement carries the caller's own image id", () => {
  const control = escapes(join(encodeKitty(small, { columns: 1, rows: 1, imageId: 777 })))[0]!.control;
  expect(control).toContain("i=777");
});

// The scoped delete: `d=i` targets exactly one id, unlike kitty's `d=A`
// ("delete every placement on the terminal"), which a second image canvas
// open alongside this one would have no protection from.
test("encodeKittyClear deletes only the given id, not every placement", () => {
  expect(encodeKittyClear(777)).toBe("\x1b_Ga=d,d=i,i=777,q=2\x1b\\");
  expect(encodeKittyClear(777)).not.toContain("d=A");
});

test("imageIdFor is stable for the same canvas id and differs across canvases", () => {
  expect(imageIdFor("image-left")).toBe(imageIdFor("image-left"));
  expect(imageIdFor("image-left")).not.toBe(imageIdFor("image-right"));
  // Never zero: kitty's `i=0` is not a valid image id.
  expect(imageIdFor("image-left")).toBeGreaterThan(0);
  expect(imageIdFor("")).toBeGreaterThan(0);
});
